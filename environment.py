...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 21 11:30:29 2017
...
ac = AtomConst(
               ...
               np.array([-0.0065, 0.0, 0.001, 0.0028, 0.0, -0.0028, -0.002, 0.0]),
               np.array([288.15, 216.65, 216.65, 228.65, 270.65, 270.65, 214.65, 186.95]),
               np.array([101325, 22632, 5474.9, 868.02, 110.91, 66.939, 3.9564, 0.3734])
               )
...
def std_atmosphere(alt):
    ...
    中間圏高度
    86kmまでの気温に対応している. それ以上は国際標準大気に当てはまらないので注意.
    ...
    # TODO: ATMOSHERIC and SPACE FLIGHT DYNAMICSより
    # TODO: Standard ATMOSPHEREのスクリプトに変更して高度2000kmまでに対応する.
    ...
    if ac.lr[_k] != 0:
        ...
        press = ac.p0[_k] * exp(ac.g / ac.gas_cnst * (ac.hal[_k] - alt) / ac.t0[_k])
...
def gravity(alt, lat):
    ...
    地球を自転周りの回転楕円体と仮定し, Jefferyの帯球係数を第二項まで考慮した
