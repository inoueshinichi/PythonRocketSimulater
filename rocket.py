...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 17 23:34:17 2017
...
rocket_settings = {
     ...
     'CD_tbl': np.array([0.28, 0.28, 0.28, 0.29, 0.35, 0.64, 0.67, 0.69,
                         0.66, 0.62, 0.58, 0.55, 0.48, 0.42, 0.38, 0.355, 0.33])
}
...
class RocketSim:
    ...
    def __init__(self, **kwargs):
        ...
        (1) 常微分方程式に使う状態量ベクトルの初期値 x0 の設定(14次元)
        ...
        pos0 = [0.0, 0.0, 0.0] # m 3次元
        vel0 = [0.0, 0.0, 0.0] # m/s 3次元
        quat0, _ = qt.attitude(kwargs['roll'], kwargs['pitch'], kwargs['yaw']) # 4次元
        omega0 = [0.0, 0.0, 0.0] # rad/s 3次元
        self.x0 = np.array([kwargs['m0'], *pos0, *vel0, *quat0, *omega0]) # 14次元
        # (2) ロケットの各種諸元設定
        self.isp = kwargs['Isp']
        self.g0 = kwargs['g0']
        ...
        self.izzdot = kwargs['IZZdot']
        self.xr, self.yr, self.zr = cc.blh2ecef(kwargs['lat0'], kwargs['lon0'], kwargs['alt0'])
    ...
    def rocket_dynamics(self, x, t, u):
        ...
        px, py, pz = cc.launch2ecef(x[1], x[2], x[3], self.xr, self.yr, self.zr)
        ...
        # 空気力
        # 水平座標系における風ベクトル VWH[m/s] (高度方向の分布は無いものとする)
        ...
        # 機体座標系から速度ベクトル vab を求めて速度座標系の定義から
        # 機体座標系からみた速度座標系の基底ベクトル[xAB yAB zAB]を求めて
        # 速度座標系から機体座標系への方向余弦行列 DCM_A2B を求めている.
        # 機体座標系における空気力 fab [N]
        ...
        faa = -0.5 * rho * (np.linalg.norm(va) ** 2) * self.area * np.array([cd, 0, self.cla * theta])
        
        dcm_a2b = np.c_[xab, yab, zab].T # 転置に注意
        fab = dcm_a2b @ faa # @は行列積(Python3.5以上)
    ...
    def plot_rs(self, tv, res):
        ...
        def plot_vel(t, d):
            ...
            plt.subplot(4, 1, 1)
            plt.plot(t, d[:, 4]) # d[インデックス, 特定の状態量]
            plt.xlabel('時間[s]')
        ...
        def plot_map(llh_in):
            ...
            minlon, maxlon = 130, 132.01 # 経度
            minlat, maxlat = 31, 32.01 # 緯度
            plt.subplot(3, 1, (1, 2))
        ...
        plot_pos(tv, res) # 位置
        plot_vel(tv, res) # 速度
        plot_vel(tv, res) # クォータニオン
        plot_rpy(tv, res) # 角速度
        plot_mass(tv, res) # 機体質量
        llh = get_llh(res)
        plot_llh(tv, llh) # 緯度, 経度, 高度
        plot_map(llh) # 地図
        plt.show()
